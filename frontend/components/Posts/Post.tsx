...
export default function Post(props: { post: IPost, setModal: (post: IPost) => void }) {
  return (
    <div onClick={() => props.setModal(props.post)} className="relative group hover:cursor-pointer">
        <img className='object-cover rounded-lg' src={props.post.image} alt={`Photo uploaded by ${props.post.owner}`} />
        <div className="text-gray-300 absolute h-full w-full bottom-0 rounded-lg bg-gradient-to-t from-black/30 group-hover:bg-black/10 duration-75">
            <div className='w-full h-full relative'>
                <a href={`/user/${props.post.owner}`} className="absolute bottom-0 left-0 flex flex-row pb-2 pl-2">
